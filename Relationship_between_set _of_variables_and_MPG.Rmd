...
While initial analysis shows that manual transmissions were better for MPG, but regression analysis showed MPG mostily depends on weight and horsepower and transmission was not major factor in determining MPG. Data shows Manual transmissions have an approximately 4-7 MPG advantage over automatic transmissions, again more significant variables are weight and horsepower. 
...
But there are 10 predictor variables in the data set,so a analysis of variance model is performed, see Table 1.2. The results showed that MPG was most highly correlated with weight (-0.87), the number of cylinders (-0.85), displacement (-0.85), and horsepower (-0.78).
